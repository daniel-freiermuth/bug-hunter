...
```sh
just build          # from hunter-rs/: bundle + release binary
npm run build       # bundle only
```

Vite writes to `../ui/` (`emptyOutDir: true`), which the Rust binary
serves as static files. That directory is a build artifact and is
gitignored — do not commit it. The target host has no node toolchain,
so the bundle is built here and shipped beside the binary; `just build`
does both in the right order, which is why it is the deploy command.
