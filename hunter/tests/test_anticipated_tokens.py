"""Tests for hunter.scheduler._anticipated_tokens.
...
runner.py's watchdog, which can only react between calls, never mid-
call). _anticipated_tokens gives budget.decide() a realistic pre-start
...
def test_no_history_returns_zero(store: Store) -> None:
    rid = store.add_repo("r", "https://r", "/r")
    assert _anticipated_tokens(store, rid, "dep_update") == 0


def test_cold_uses_p90_not_p50(store: Store) -> None:
...
def test_warm_uses_median_not_p90(store: Store) -> None:
...
def test_warmth_is_scoped_to_this_repo(store: Store) -> None:
    ...
    cold_estimate = _anticipated_tokens(store, rid_a, "dep_update")
    ...
    still_cold_estimate = _anticipated_tokens(store, rid_a, "dep_update")
...
def test_kind_scoped_independently(store: Store) -> None:
