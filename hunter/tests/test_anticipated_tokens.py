...
from hunter.scheduler import CACHE_TTL_MS, anticipated_tokens
...
def test_no_history_returns_zero(store: Store) -> None:
    ...
    assert anticipated_tokens(store, rid, "dep_update") == 0
...
def test_cold_uses_p90_not_p50(store: Store) -> None:
    ...
    anticipated = anticipated_tokens(store, rid, "dep_update")
...
def test_warmth_is_scoped_to_this_repo(store: Store) -> None:
    ...
    cold_estimate = anticipated_tokens(store, rid_a, "dep_update")
    ...
    still_cold_estimate = anticipated_tokens(store, rid_a, "dep_update")
...
def test_kind_scoped_independently(store: Store) -> None:
    ...
    for i, t in enumerate([1_000] * 9 + [700_000]):
        _finished_job(store, rid, "dep_update", t, old - i * 1000)
    assert anticipated_tokens(store, rid, "hunt") == 0
