...
`ui_dir/index.html` from disk per request (`static_files` in
hunter-rs/src/server.rs), so a restart
...
(also in `static_files`) disappear, and there is no directory to be missing.
