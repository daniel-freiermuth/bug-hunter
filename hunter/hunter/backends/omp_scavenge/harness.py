...
def _run_session_dir(work_root: Path, cwd: Path, spawn_ms: int) -> Path:
    ...
    # One path component, and a component is capped at NAME_MAX (255
    # bytes on Linux), so a deep enough work_root makes mkdir fail with
    # ENAMETOOLONG before omp is ever spawned -- surfacing as an
    # unmetered worker, i.e. a path length reported as a budget failure.
    # Keep the tail: the end is what distinguishes one worktree from
    # another, so truncating the head would collide across findings.
    slug = slug[-150:]
