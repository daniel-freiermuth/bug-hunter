...
ALTER TABLE repos ADD COLUMN last_full_hunt_at INTEGER;                       -- MIGRATION-ONLY
ALTER TABLE repos ADD COLUMN last_test_gap_at INTEGER;                        -- MIGRATION-ONLY
ALTER TABLE repos ADD COLUMN last_dep_update_at INTEGER;                      -- MIGRATION-ONLY
ALTER TABLE repos ADD COLUMN last_refactor_at INTEGER;                        -- MIGRATION-ONLY
ALTER TABLE repos ADD COLUMN last_modernization_at INTEGER;                   -- MIGRATION-ONLY
ALTER TABLE findings ADD COLUMN modernization_class TEXT;                     -- MIGRATION-ONLY
ALTER TABLE findings ADD COLUMN current_approach TEXT;                        -- MIGRATION-ONLY
ALTER TABLE findings ADD COLUMN proposed_approach TEXT;                       -- MIGRATION-ONLY
...
The eight MIGRATION-ONLY columns exist on every real DB but NOT in schema.sql — the Rust port's schema must include them (they surface in `/api/repos` and `/api/findings` via `SELECT *`).
