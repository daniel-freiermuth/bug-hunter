...
- **Phase two (`reap_repo`, server.py:751-772; server.rs:1038-1055)**: remove `<work_root>/repos/repo-<id>` entirely — clone, `NOTES.md`, any worktrees under it — and only then `DELETE` the row (`forget_deleted_repo`, itself `WHERE deleted_at IS NOT NULL`). That delete is the moment the repo stops being tracked at all, so it must not run first: a removal that fails or is interrupted has to stay findable. Missing directory is success. Reclamation wants the notes lock, because a note written between the `rmtree` and the row delete recreates the directory after the reaper has passed it — leaving files behind with no row left to retry them. **The two daemons hold it differently**: Python takes it inside `reap_repo` itself (server.py:764), so every caller is covered; Rust takes it in the delete handler (server.rs:1083), so only the inline attempt is — `reap_deleted_repos` (server.rs:1058-1073) walks the flagged rows with no lock held, and that is the pass that runs at startup and before every cycle.
