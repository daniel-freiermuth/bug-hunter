...
//! - Python's commit-per-method transaction model is preserved:
//!   no multi-statement transactions in handlers.
...
/// Tail-truncation limit for repo notes (store.py:278-294).
...
impl Store {
    ...
    pub async fn count_enabled_repos(&self) -> sqlx::Result<i64> {
        sqlx::query_scalar!(r#"SELECT COUNT(*) AS "n!: i64" FROM repos WHERE enabled = 1"#)
            .fetch_one(&self.pool)
            .await
    }
...
impl Store {
    ...
    /// instead") when findings or jobs reference the repo; else
    /// DELETE FROM repos WHERE id = ?. Filesystem untouched.
    pub async fn delete_repo(&self, id: i64) -> Result<(), StoreWriteError> {
        ...
        sqlx::query!("DELETE FROM repos WHERE id = ?1", id)
            .execute(&self.pool)
            .await?;
        Ok(())
    }
    ...
    pub async fn events_by_finding(&self) -> sqlx::Result<BTreeMap<i64, Vec<Event>>> {
    ...
    // -- scheduler store methods (round 3) ------------------------------------
    ...
    /// INSERT OR IGNORE finding by fingerprint. Returns (id, `was_inserted`).
    ...
    pub async fn reconcile_orphaned_jobs(&self) -> sqlx::Result<(Vec<Finding>, Vec<Job>)> {
        ...
        let all_jobs = self.list_jobs(i64::MAX).await?;
...
impl crate::backend::SpendLedger for Store {
    ...
    async fn finished_since(&self, ts_ms: i64) -> sqlx::Result<i64> {
        sqlx::query_scalar!(
            r#"SELECT COALESCE(SUM(tokens_new), 0) AS "total!: i64"
               FROM jobs WHERE state != 'running' AND finished_at > ?1"#,
            ts_ms
        )
        .fetch_one(&self.pool)
        .await
    ...
    async fn finished_between(&self, start_ms: i64, end_ms: i64) -> sqlx::Result<i64> {
        sqlx::query_scalar!(
            r#"SELECT COALESCE(SUM(tokens_new), 0) AS "total!: i64"
               FROM jobs
               WHERE state != 'running' AND finished_at > ?1 AND finished_at <= ?2"#,
            start_ms,
            end_ms
        )
        .fetch_one(&self.pool)
    ...
    async fn log_window_observation(
        ...
        // int(age_s): truncation toward zero, matching Python (store.py:961).
    ...
    async fn estimate_capacity(&self, limit_id: &str) -> sqlx::Result<Option<f64>> {
        ...
        let now = now_ms();
        // Distinct completed cycles, deduping resets_at to 10 s buckets.
        let cycles = sqlx::query!(
            r#"SELECT CAST(resets_at / 10000 AS INT) AS "cycle_key: i64",
                      MIN(resets_at) AS "resets_at: i64"
               FROM window_log
        ...
        let mut best: i64 = 0;
        for cycle in cycles {
            let Some(resets) = cycle.resets_at else {
                continue;
            };
            let start = resets - period_ms;
            let spent = sqlx::query_scalar!(
                r#"SELECT COALESCE(SUM(tokens_new), 0) AS "t!: i64"
                   FROM jobs
