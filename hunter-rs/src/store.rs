...
impl Store {
    ...
    /// Drop a flagged row once its directory is gone. This is the moment
    /// the id becomes available again, so it must not run before the
    ...
    /// the id list too (`events_by_finding(fids)`, store.py:861-877).
    ///
    ...
    ///
    /// Python selected `status IN ACTIVE_STATUSES` (types.py:68-76), which
