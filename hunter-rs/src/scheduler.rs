//! Scheduler — round 2 ports ONLY the pure selection half used by
//! GET /api/summary's next-candidate preview: `pick_next` + `anticipated_tokens`
//! (scheduler.py:39-77, 1824-1925). run_* executors arrive in round 3.
...
/// (scheduler.py:39-77, exact index formula in BACKEND-CONTRACT.md §1.9).
...
// ---------------------------------------------------------------------------
// Round 3: executor functions ported from scheduler.py
// ---------------------------------------------------------------------------
...
pub async fn run_recheck(
    ...
    let verdict_str = match outcome {
        ...
        RecheckOutcome::Unknown(_) => {
            let failure = format!("invalid verdict value: {outcome:?}");
            return Ok(handle_recheck_failure(
                store,
                &mut summary,
                fid,
