...
//! (`decide`); (2) run this job (`run` — ROUND 3, deliberately absent from
//! the trait until the runner ports); (3) what's your status
//! (`keep_fresh`/`status_html`).
//!
...
/// at the backend boundary (backend.py:27-35).
...
/// Paired verdicts (backend.py:68-83).
///
...
/// history (backend.py:87-146). Implemented by Store on the sqlx pool
...
pub trait SpendLedger: Send + Sync {
    ...
    /// (NOT p75 — backend.py:142's docstring is stale; the Python
    /// `min_delta` param is dead and dropped here). None for unknown
...
/// The backend facade — accounting + budget policy (+ harness in round 3).
///
