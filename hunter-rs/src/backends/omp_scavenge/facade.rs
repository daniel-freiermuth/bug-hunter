...
/// 200 k ≈ 10% of a 5h window (facade.py:34-37).
const TOK_PER_FRAC_5H: f64 = 2_000_000.0;
/// 5h / 7d period ratio ≈ 0.0297619 (facade.py:38).
const RATIO_5H_7D: f64 = FIVE_H_MS as f64 / WEEK_MS as f64;

/// Calibration horizons: iteration order "5h" then "7d" (facade.py:50-53).
...
impl OmpScavengeBackend {
    /// Fraction → token cap (facade.py:219-225).
    ///
    ...
    /// Inflight reservation as fraction per window (facade.py:68-98).
    ...
    /// Single verdict: 7d ramps first, then 5h (facade.py:100-176).
    fn decide_inner(
        windows: &BTreeMap<String, WindowState>,
        resv: Reservation,
        prio: bool,
    ...
    /// Min headroom in tokens across windows (facade.py:178-212).
    ///
    ...
    /// Log observations + record calibration (facade.py:306-346).
    ...
    /// Max `used_fraction` across 7d windows, or None (facade.py:270-278).
