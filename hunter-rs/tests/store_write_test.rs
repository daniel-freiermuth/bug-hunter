...
async fn delete_repo_refusal_message_is_byte_exact_and_clean_delete_works() {
    ...
    let err = store.delete_repo(1).await.unwrap_err();
    match err {
    ...
    // Clean repo deletes; the guarded one survives.
    store.delete_repo(2).await.unwrap();
...
async fn add_repo_defaults_and_update_repo_partial_fields() {
    ...
    assert_eq!(r.path, "/tmp/wr/repos/gamma");
