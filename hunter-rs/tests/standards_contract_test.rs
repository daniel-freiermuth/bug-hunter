...
//! to emit; `ingest::type_required_fields` decides which are mandatory. If
//! those two disagree, every standards finding is counted invalid and
//! silently dropped — which is what shipped: ingest demanded refactor's
//! `smell_type`, a key the standards playbook never mentions.
...
async fn ingest_one(entry: &serde_json::Value) -> (Store, hunter::ingest::IngestResult, i64) {
...
async fn a_playbook_shaped_standards_finding_is_accepted() {
    let (store, res, _repo) = ingest_one(&playbook_entry()).await;
    assert_eq!(
        res.invalid, 0,
...
async fn a_standards_finding_without_its_section_is_invalid() {
    ...
    let (_store, res, _repo) = ingest_one(&e).await;
