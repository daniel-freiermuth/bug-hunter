...
//! HTML-escaped.
//!
//! It had no test. The check used to be `scripts/parity.sh`, which
//! diffed this endpoint against the Python daemon during the port; that
//! script was deleted with the Python daemon, taking the only assertion
//! on ~115 lines of specified rendering with it. Snapshots replace it —
//! the contract is written in HTML, so the diff should be too.
//!
//! Deterministic by construction: `render_status` takes the clock and
//! every database-derived value as input, and the one piece that used to
//! be machine-dependent (a server-local 12-hour reset time) is now
//! emitted as `<time data-ms>` for the browser to format.
