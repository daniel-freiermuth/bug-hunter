...
//! and only closes once the comment lands.** It used to discard the
//! comment result and close regardless, turning a forge outage into a
//! silently unexplained closed PR. Two further input classes shipped
//! broken alongside it and are pinned below: an *empty* reason (skipped,
//! so a comment failure cannot block the close) and a *multibyte* reason
//! (truncated by characters — truncating by bytes panics mid-codepoint).
