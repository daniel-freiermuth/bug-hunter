...
| POST `/api/cycle` | server.py:489-518: spawns `scheduler.run_cycle(store, cfg, backend=backend)` in a thread under `_cycle_lock` | full scheduler + `run()` — both landed (§6): `scheduler::run_cycle` scheduler.rs:3818, `Backend::run` backend.rs:161-169 |
...
Scheduler call sites (all ported to `src/scheduler.rs`; they define `decide()`'s contract): run_hunt scheduler.py:472-478 (`outlook.normal`; hunts have no override), run_recheck :641-647, run_fix :1230-1242, run_engage :1824-1834, run_harvest :2139-2149 (all four: `verdict = outlook.prioritized if override else outlook.normal`; `override = finding.get("budget_override")`, values `'once'|'exempt'|None`, store.py:972-981; `'once'` is cleared after any attempt), `_run_analysis_job` scheduler.py:854-860 (normal). Each site now takes its `anticipated` from the resume plan when it is continuing a suspension, and `anticipated_tokens(store, cfg, …)` otherwise. Uniform pattern:
- `Denied(reason, retry_at)` → `log_event('deny', …)`; return `{"denied": reason, "retry_at": retry_at}` — no job row is written (e.g. scheduler.py:475-477). These writes are the **caller's**, not the backend's.
- `Granted(cap_tokens=backend_cap)` → `cap = backend_cap`, verbatim, `None` included (scheduler.py:478,647,860,1242,1834,2149; scheduler.rs:937-948). The ramp's headroom is the job's only token bound, and `None` means it has none: the job runs under `maxWallS` alone and its row stores NULL in `jobs.cap_tokens`. **There is nothing for the core to min() against.** The `hunt.capNewTokens` / `fix.capNewTokens` constants that used to bound this are gone (§3): a static number beside the ramp is a second guess at a quantity the ramp computes from live window state, and the one that shipped — 200 000 for hunts — had drifted below the measured typical cost of a hunt (~204 000), so it was killing jobs by itself. Across 85 cap-kills, 96% ran under a cap below the scheduler's own estimate of what the job would cost.
...
def run(self, cwd: Path, prompt: str, *, cap_tokens: int | None,
        ...
        resume_from: Path | None = None) -> RunResult        # :177-196  (Rust: backend.rs:161-169)
def keep_fresh(self) -> bool                                 # :198-207
def status(self) -> str                                      # :209-221
```
- `decide`: `anticipated_tokens` = caller's pre-reservation for the job under decision (realistic historical estimate, cache-warmth aware). Returns both verdicts; each Granted carries the backend's own cap (backend.py:166-174).
...
- `keep_fresh`: refresh stale accounting, log observations, record calibration. Called from the server's dedicated prober thread on a fixed tick, decoupled from dispatch cadence. Returns True iff a refresh was performed (:199-206).
...
Warm iff this exact `(repo_id, kind)` finished a non-denied job within `cfg.cache_ttl_s`: `SELECT 1 FROM jobs WHERE repo_id = ? AND kind = ? AND finished_at > ? AND state != 'denied' LIMIT 1` (cutoff `now_ms() − cache_ttl_s*1000`). History = `tokens_new` for the kind from `state = 'done'` rows only, ascending (`scheduler._kind_token_history`; Rust `Store::kind_token_history`) — a killed job's `tokens_new` records the bound that killed it, not the cost of the work, so killed rows cluster at the cap and the percentile measures the estimator's own failures. Below 3 completed rows the query falls back to the unfiltered history, because an empty history estimates 0 and an `anticipated` of 0 makes the gate reserve nothing. Empty → 0; else `history[min(int(len * (0.5 if warm else 0.9)), len−1)]` — warm p50, cold p90.
