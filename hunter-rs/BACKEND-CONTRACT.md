...
```
- `decide`: `anticipated_tokens` = caller's pre-reservation for the job under decision (realistic historical estimate, cache-warmth aware). Returns both verdicts; each Granted carries the backend's own cap (backend.py:165-175).
...
The reservation handed to `decide` is `ctx_at_suspension + max(z − chain_spent, 25 000)`, where `z` is §1.9's per-kind estimate, `chain_spent` is the `total` of `Store::resume_chain_stats(predecessor)` — one recursive walk over the chain returning `(total, attempts, max_single)`: everything the chain has spent, how many attempts make it up, and the priciest single attempt — and `ctx_at_suspension` is the last usage record's `input + cacheRead + cacheWrite` in the predecessor's transcript (`harness.ctx_at_suspension`; falls back to `z` when the file is unreadable). The floor exists because a chain that has already outspent `z` makes `z − chain_spent` negative, and a negative budget for the remaining work would fund a resume that can only re-cache and be killed again.
...
- `attempts >= MAX_RESUME_ATTEMPTS` (4). The arm that actually bounds the loop: a chain whose every attempt is cheap — resume, do a little, suspend again — never trips a spend threshold at all. It is also the arm a reader can reason about, because it is expressed in the unit the question is really asked in, how many times have we tried this.
...
Subtracting the largest attempt rather than multiplying it is forced, not stylistic: `chain_spent <= attempts * largest` always holds, so a threshold of the form `k * largest` is unreachable below `attempts = k + 1`. At `k = 3` the multiplying form could never fire for a chain of two or three — precisely the range this arm exists to cover — which made it dead code. Both arms leave a chain of one alone: a first suspension is a single attempt whose spend is, by definition of a cap kill, at least its own cap, and retiring on that would end the work before resume had been tried even once. The retirement happens in the selection tier itself (`pick_resume`), not by refusing inside the executor: a refusal would leave the row `suspended`, and because this tier outranks repo rotation the same hopeless chain would be offered again every cycle, starving every rotation kind.
