...
**`running_estimate() -> int`** (proto backend.py:98-100; impl store.py:923-928). Per job this is `estimated_tokens` — what the ramp reserved when it granted the job — not `cap_tokens`, which is only a kill threshold and may not be finite. Rows written before `estimated_tokens` existed fall back to their cap.
```sql
...
**`estimate_capacity(limit_id, min_delta=0.02, sample_limit=200) -> f64 | None`** (backend.py:139-142; store.py:878-921; Rust `Store::estimate_capacity`, store.rs:1994-2046, where `sample_limit` is the constant `SAMPLE_LIMIT = 200` and `min_delta` is not in the signature at all). ⚠️ **Docstring drift**: backend.py:142 says "p75 estimate"; the implementation is **max tokens hunter ever spent in one completed window cycle** (store.py:887-891) — no fraction correlation, no p75, and **`min_delta` is dead** (never referenced in store.py:884-921; ThreadLocalLedger just forwards it, store.py:1136-1139). Keep the param for signature parity or delete in both. Algorithm:
1. `period_ms = {"anthropic:5h": 18_000_000, "anthropic:7d": 604_800_000}.get(limit_id)` (`_PERIOD_MS` store.py:878-882; store.rs:1997-2002); unknown → `None` (:893-895). Called with per-model-class lids (from status()) it correctly returns None.
2. **One statement** (store.rs:2014-2045), not one per cycle: a `cycles` CTE picks the completed cycles from `window_log`, deduping `resets_at` into 10 s buckets and keeping the newest `sample_limit`; a correlated scalar subquery sums hunter's spend inside each cycle's half-open `(resets − period_ms, resets]` window; the outer `MAX` takes the winner. Python still runs the cycle query and then one SUM per cycle (store.py:897-917) — same answer, N+1 round trips.
```sql
WITH cycles AS (
...
def run(self, cwd: Path, prompt: str, *, cap_tokens: int | None,
        ...
        resume_from: Path | None = None) -> RunResult        # :177-195  (Rust: backend.rs:161-169)
...
| anything else (`"wallclock"`, `"unmetered"`, …) | — | `killed` |
| `None`, `exit_code == 0` | — | `done` |
| `None`, otherwise | — | `failed` |

A cap kill that left a transcript is a **pause**: the worker ran out of window headroom mid-thought and the reasoning that got it there is still on disk. Continuing costs the context at suspension (measured median re-cache / prior-context ratio 1.00, p10 1.00, across 112 production re-cache events); restarting costs a flat ~37 000-token session floor — call #1 `cacheWrite`, measured at ~37 k even in a 4 255-call session — plus every token already spent. Recording it as `killed` is what let one repo run ten consecutive hunts over an identical diff range for 1.48 M tokens, eight producing nothing: a killed job advances no watermark, so the same work was re-selected from scratch every cycle. The session file is load-bearing, not incidental — resuming means handing omp one exact path. `wallclock` is never a suspension: an unbounded overrun is the runaway signature, and continuing a runaway only buys it more wall clock. `resume-unavailable` is `failed` rather than `killed` because nothing was spawned.

**What the scheduler does with a suspension.** Selection (`pick_next`) offers resumable work after every finding-driven tier — those are all a human waiting on a PR — and before repo rotation, since a resume is background work already paid for. `Store::list_resumable_jobs` supplies the candidates: `state = 'suspended'`, repo live, `session_file` non-NULL, and `NOT EXISTS` a row whose `resumed_from` names it (the successor row IS the record that the work was picked up, which is why there is no `resumed` state). The scheduler additionally requires the job's working directory to still exist — `repo.path` for hunt/recheck/analysis kinds, `<work_root>/wt/{f,e,h}<finding_id>` for fix/engage/harvest — because a resumed worker continues a conversation, not a filesystem.

The reservation handed to `decide` is `ctx_at_suspension + max(z − chain_spent, 25 000)`, where `z` is §1.9's per-kind estimate, `chain_spent` is `Store::resume_chain_tokens(predecessor)`, and `ctx_at_suspension` is the last usage record's `input + cacheRead + cacheWrite` in the predecessor's transcript (`harness.ctx_at_suspension`; falls back to `z` when the file is unreadable). The floor exists because a chain that has already outspent `z` makes `z − chain_spent` negative, and a negative budget for the remaining work would fund a resume that can only re-cache and be killed again. Above `chain_spent > 3z` the chain is abandoned instead: the predecessor is retired `failed` with `killed_reason = "give-up"` and selection falls through.

Executing a resume reuses the suspended kind's own executor, so ingest, watermarks and PR handling are unchanged. The differences: the prompt is exactly `Continue the work you were doing in this session. You were interrupted; pick up where you left off.` (the session already holds the playbook); `resume_from` is the predecessor's `session_file`; the new row carries `resumed_from`; the repo is **not** synced (fast-forwarding moves the tree under a live session, and `run_hunt` writes the HEAD it reads as the watermark, so a resume that pulled would mark commits reviewed that no worker saw); and job-id-keyed output paths use the chain's first attempt (`Store::resume_origin_job`), since the continuing worker still writes the path its original prompt named. On `resume-unavailable` the new attempt is recorded `failed` and the predecessor is retired `killed` — never restarted cold in the same cycle, because omp would then pay a full session floor while the daemon believed it had continued.

### 1.9 What feeds `anticipated_tokens` (scheduler.py:45-111 — stays in core)
Warm iff this exact `(repo_id, kind)` finished a non-denied job within `cfg.cache_ttl_s`: `SELECT 1 FROM jobs WHERE repo_id = ? AND kind = ? AND finished_at > ? AND state != 'denied' LIMIT 1` (cutoff `now_ms() − cache_ttl_s*1000`). History = `tokens_new` for the kind from `state = 'done'` rows only, ascending — a killed job's `tokens_new` records the bound that killed it, not the cost of the work, so killed rows cluster at the cap and the percentile measures the estimator's own failures. Below 3 completed rows the query falls back to the unfiltered history, because an empty history estimates 0 and an `anticipated` of 0 makes the gate reserve nothing. Empty → 0; else `history[min(int(len * (0.5 if warm else 0.9)), len−1)]` — warm p50, cold p90.
...
**`omp_scavenge`** (`src/backends/omp_scavenge/`): `capacity.rs` (`read_windows`, the `ramp_7d`/`ramp_5h` + `retry_at_*` pair, `effective_used`), `facade.rs` (`impl Backend for OmpScavengeBackend`) including `_usage_snapshot_sync` taken either side of a worker run, and `harness.rs` (`run_worker` spawn+meter, `run_session_dir`/`ledger_dir_usage` for the run's private session directory, `ledger_usage`, and the SIGTERM→SIGKILL group kill `util::kill_tree` (util.rs:65)). `NullBackend` (backend.rs:164-197) remains for router tests only: its `run()` errors by construction, which is the point.
